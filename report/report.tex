...
Appurato il funzionamento e l'efficacia degli algoritmi ECM e K-Means è stato studiato un'approccio più dinamico e flessibile, al fine di evitare il processo manuale di categorizzazione della fase 1 e per parametrizzabile, dunque facile da impostare, l'ottimizzazione di ogni comparatore.\\
...
\begin{enumerate}
	\item Scelta di un dataset \textit{i} tra gli \textit{N} datasets in input, da cui partirà il processo. La scelta può avvenire in modo randomico, oppure scegliendo il dataset che massimizza un determinata misura di qualità dei dati (es. completezza), per ottimizzare le fasi successive del processo.
